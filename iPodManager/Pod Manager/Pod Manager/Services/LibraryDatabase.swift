...
class LibraryDatabase {
    ...
    private func openDatabase() {
        if sqlite3_open(dbPath, &db) != SQLITE_OK {
            print("LibraryDatabase: Failed to open database")
        } else {
    ...
    func saveTracks(_ tracks: [Track], isPodcast: Bool = false) {
        print("LibraryDatabase: Saving \(tracks.count) tracks")
        ...
        guard sqlite3_prepare_v2(db, insertSQL, -1, &statement, nil) == SQLITE_OK else {
            let error = String(cString: sqlite3_errmsg(db))
            print("LibraryDatabase: Failed to prepare: \(error)")
            return
        }
        ...
        print("LibraryDatabase: Saved \(savedCount) tracks")
    }
    ...
    func loadTracks(isPodcast: Bool = false) -> [Track] {
        ...
        print("LibraryDatabase: Loaded \(tracks.count) tracks")
        return tracks
    }
